...
function ColorSelector({ selectedColor, onColorChange }) {
  ...
  return (
    <>
      ...
      <div className="flex gap-3.5 mt-6 ml-4 md:ml-2.5 sm:flex-col md:flex-row sm:items-start">
  {colors.map((color) => (
    <button
      key={color.id}
      className={`h-6 w-6 sm:h-6 sm:w-6 md:h-12 md:w-12 shrink-0 border border-black ${
